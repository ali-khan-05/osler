...
import { effectiveApiKey, effectiveModel } from './settings'
import { log } from './log'
...
async function generateBatch(
  lecture: Anthropic.ContentBlockParam,
  batchCount: number,
  part: number,
  totalParts: number
): Promise<Question[]> {
  ...
  const stream = getClient().messages.stream({
    ...
    messages: [
      {
        role: 'user',
        content: [lecture, { type: 'text', text: prompt }]
      }
    ]
  })
  stream.on('error', (err) =>
...
export async function generateQuestions(filePath: string, count: number): Promise<Question[]> {
  const sizeMb = fs.statSync(filePath).size / 1024 / 1024
  log.info('generate', `Reading ${basename(filePath)} (${sizeMb.toFixed(1)} MB) · ${count} questions · ${MODEL()}`)
  const lecture = fileToContentBlock(filePath)
  ...
  log.info('generate', `Part 1/${parts} done (${first.length} questions) — generating the rest in parallel…`)
  let done = 1
  const rest = await Promise.all(
    sizes.slice(1).map((batchCount, i) =>
      generateBatch(lecture, batchCount, i + 2, parts).then((qs) => {
        done += 1
        log.info('generate', `Part ${done}/${parts} done (${qs.length} questions)`)
        return qs
      })
    )
  )
  const questions = [first, ...rest].flat()
  log.success('generate', `Created ${questions.length} questions`)
  return questions
}
...
Correct answer: ${optionLetters[q.correctIndex]}. ${q.options[q.correctIndex]}${
  ...
  const response = await getClient().messages.create({
    ...
    max_tokens: 16000,
    thinking: THINKING(),
    system: `You are a supportive medical school study coach. A student just completed a practice question set generated from their lecture slides. Analyze their performance and tell them what to go back and study.
...
Be specific to the actual content — name mechanisms, drugs, pathways from the questions. Never be generic.`,
    messages: [
      {
        role: 'user',
        content: `Here are my results on "${set.title}" (${set.questions.length} questions):\n\n${results}`
      }
    ]
  })
